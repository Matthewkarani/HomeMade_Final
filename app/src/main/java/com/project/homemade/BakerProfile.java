...
public class BakerProfile extends AppCompatActivity {
    ...
    private EditText BakerUserName;
    private ImageButton imageButton;
    private Button doneBtn;
    // Declare an instance of firebase authentication
    private FirebaseAuth mAuth;
    //Declare an Instance of the database reference where we will be saving the profile photo and custom
    // display name
    ...
    // since we want to get a result (getting and setting image) we will start the implicit intent
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        BakerUserName = findViewById(R.id.BakerUserName);
        mDescription = findViewById(R.id.Description);
        imageButton = findViewById(R.id.imgbtn);
        doneBtn = findViewById(R.id.buttonNext);
        ...
        //Initialize the database reference where you have your registered users and get the specific
        // user reference using the user ID
        ...
        mStorageRef = FirebaseStorage.getInstance().getReference().child("baker_profile_images");
        //set on click listener on the image button so as to allow users to pick their profile photo
        // from their gallery
        ...
        // on clicking the images we want to get the name and the profile photo, then later save this on
        // a database reference for a specific user
        doneBtn.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //get the custom display name entered by the user
                final String name = BakerUserName.getText().toString().trim();
                ...
                if (!TextUtils.isEmpty(name) && profileImageUri != null) {
                    ...
                    StorageReference profileImagePath = mStorageRef.child("baker_profile_images").child(profileImageUri.getLastPathSegment());
                                        ...
                                        public void onSuccess(Uri uri) {
                                            ...
                                            mDatabaseUser.addValueEventListener(new ValueEventListener() {
                                                ...
                                                public void onDataChange(@NonNull DataSnapshot snapshot) {
                                                    ...
                                                    mDatabaseUser.child(" Baker UserName").setValue(name);
                                                    mDatabaseUser.child(" Baker Description").setValue(Description);
                                                    mDatabaseUser.child("bakerprofilePhoto").setValue(profileImage).addOnCompleteListener(new OnCompleteListener<Void>() {
                                                        @Override
                                                        public void onComplete(@NonNull Task<Void> task) {
                                                            if (task.isSuccessful()) {
                                                                //show a toast to indicate the profile was updated
                                                                Toast.makeText(BakerProfile.this, "Profile Updated", Toast.LENGTH_SHORT).show();
                                                                //launch the login activity
                                                                Intent login = new Intent(BakerProfile.this, Baker_Account.class);
                                                                startActivity(login);
                                                            }
                                                        }
                                                    });
    ...
    public void Save_Location(View view) {
        Intent login = new Intent(BakerProfile.this, BakerMapActivity.class);
        startActivity(login);
    }

    public void send_to_account(View view) {
        Intent login = new Intent(BakerProfile.this, Baker_Account.class);
        startActivity(login);
    }
}
